...
export class OrdersController {
  ...
  create(@Body() dto: CreateOrderDto, @CurrentUser('sub') userId: string) {
  ...
  @ApiQuery({ name: 'search', required: false, description: 'Buscar por numero, titulo ou cliente' })
  ...
  findAll(
    ...
    return this.ordersService.findAll(pagination, { search, status, clientId, createdByUserId });
  }

  @Get(':id')
  @Roles(Role.ADMIN, Role.MANAGER, Role.OPERATOR)
  @ApiOperation({ summary: 'Dossie completo do pedido (items, supplies, historico)' })
  ...
  @Patch(':id')
  @Roles(Role.ADMIN, Role.MANAGER, Role.OPERATOR)
  @ApiOperation({ summary: 'Atualizar dados do pedido (items, pagamento, etc)' })
  @ApiResponse({ status: 200, type: OrderResponseDto })
  ...
  @ApiOperation({ summary: 'Remover pedido (soft delete, somente EM_ORCAMENTO)' })
  ...
  @Patch(':id/status')
  @Roles(Role.ADMIN, Role.MANAGER, Role.OPERATOR)
  @ApiOperation({ summary: 'Avancar status do pedido (dispara BPM engine + guards)' })
  @ApiResponse({ status: 200, type: OrderResponseDto })
  @ApiResponse({ status: 400, description: 'Transicao invalida ou guard falhou' })
  changeStatus(
    @Param('id') id: string,
  ...
  @Patch(':id/payment')
  @Roles(Role.ADMIN, Role.MANAGER, Role.OPERATOR)
  @ApiOperation({ summary: 'Registrar pagamento (paidAmountCents + paymentProofUrl)' })
  @ApiResponse({ status: 200, type: OrderResponseDto })
  ...
  addSupply(
    ...
    return this.ordersService.addSupply(orderId, itemId, dto);
  }

  @Patch(':id/items/:itemId/supplies/:supplyId')
  @Roles(Role.ADMIN, Role.MANAGER, Role.OPERATOR)
  @ApiOperation({ summary: 'Marcar supply como READY/PENDING (toggle checklist)' })
  ...
  toggleSupply(
    ...
    return this.ordersService.toggleSupply(orderId, itemId, supplyId, dto, userId);
  }
  ...
  @Roles(Role.ADMIN, Role.MANAGER, Role.OPERATOR)
  @ApiOperation({ summary: 'Gerar Etiqueta de Producao (itens FABRICACAO_PROPRIA)' })
  @ApiResponse({ status: 200, description: 'PDF binario' })
  async getPdfProductionLabel(@Param('id') id: string, @Res() res: express.Response) {
  ...
  @Roles(Role.ADMIN, Role.MANAGER, Role.OPERATOR)
  @ApiOperation({ summary: 'Gerar Etiqueta de Separacao (itens REVENDA/INSUMO)' })
  @ApiResponse({ status: 200, description: 'PDF binario (404 se nao houver itens REVENDA)' })
  async getPdfSeparationLabel(@Param('id') id: string, @Res() res: express.Response) {
