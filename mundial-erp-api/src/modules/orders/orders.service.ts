...
import { OrderStatusMachine, TransitionContext } from '../bpm/engine/order-status-machine';
...
export class OrdersService {
  ...
  async create(dto: CreateOrderDto, userId: string): Promise<OrderResponseDto> {
    ...
    if (dto.companyId) {
      ...
      if (!company) throw new BadRequestException(`Empresa "${dto.companyId}" nao encontrada`);
    }
    if (dto.paymentMethodId) {
      const pm = await this.prisma.paymentMethod.findFirst({ where: { id: dto.paymentMethodId, deletedAt: null } });
      if (!pm) throw new BadRequestException(`Forma de pagamento "${dto.paymentMethodId}" nao encontrada`);
    }

    const orderNumber = await this.ordersRepository.generateOrderNumber();

    const itemsData = (dto.items ?? []).map((item, index) => {
      const totalCents = Math.round(item.quantity * item.unitPriceCents) - (item.discountCents ?? 0);
      return {
    ...
    const totalCents = subtotalCents + freightCents - discountCents + taxSubstitutionCents;
    ...
    const createData: Prisma.OrderCreateInput = {
      ...
      ...(dto.priceTableId && { priceTable: { connect: { id: dto.priceTableId } } }),
      ...(dto.assignedUserId && { assignedTo: { connect: { id: dto.assignedUserId } } }),
      ...(dto.deliveryDeadline && { deliveryDeadline: new Date(dto.deliveryDeadline) }),
      ...(dto.orderTypeId && { orderType: { connect: { id: dto.orderTypeId } } }),
      ...(dto.orderFlowId && { orderFlow: { connect: { id: dto.orderFlowId } } }),
      ...(dto.orderModelId && { orderModel: { connect: { id: dto.orderModelId } } }),
      ...(itemsData.length > 0 && {
        items: {
            ...
            ...(item.supplies.length > 0 && {
              supplies: {
                create: item.supplies.map((s) => ({
                  ...
                  ...(s.productId && { product: { connect: { id: s.productId } } }),
                })),
              },
            }),
          })),
    ...
    const entity = await this.ordersRepository.create(createData);
  ...
  async update(id: string, dto: UpdateOrderDto): Promise<OrderResponseDto> {
    ...
    if (dto.deliveryDeadline !== undefined) updateData.deliveryDeadline = new Date(dto.deliveryDeadline);
    if (dto.proposalValidityDays !== undefined) updateData.proposalValidityDays = dto.proposalValidityDays;
    if (dto.deliveryAddress !== undefined) updateData.deliveryAddress = dto.deliveryAddress;
    if (dto.deliveryNeighborhood !== undefined) updateData.deliveryNeighborhood = dto.deliveryNeighborhood;
    if (dto.deliveryCity !== undefined) updateData.deliveryCity = dto.deliveryCity;
    if (dto.deliveryState !== undefined) updateData.deliveryState = dto.deliveryState;
    if (dto.deliveryCep !== undefined) updateData.deliveryCep = dto.deliveryCep;
    if (dto.deliveryReferencePoint !== undefined) updateData.deliveryReferencePoint = dto.deliveryReferencePoint;
    if (dto.contactName !== undefined) updateData.contactName = dto.contactName;
    if (dto.freightCents !== undefined) updateData.freightCents = dto.freightCents;
    if (dto.discountCents !== undefined) updateData.discountCents = dto.discountCents;
    if (dto.taxSubstitutionCents !== undefined) updateData.taxSubstitutionCents = dto.taxSubstitutionCents;
    if (dto.shouldProduce !== undefined) updateData.shouldProduce = dto.shouldProduce;
    if (dto.isResale !== undefined) updateData.isResale = dto.isResale;
    if (dto.hasTaxSubstitution !== undefined) updateData.hasTaxSubstitution = dto.hasTaxSubstitution;
    if (dto.notes !== undefined) updateData.notes = dto.notes;

    if (dto.companyId !== undefined) {
      updateData.company = dto.companyId ? { connect: { id: dto.companyId } } : { disconnect: true };
    }
    if (dto.paymentMethodId !== undefined) {
      updateData.paymentMethod = dto.paymentMethodId ? { connect: { id: dto.paymentMethodId } } : { disconnect: true };
    }
    if (dto.carrierId !== undefined) {
      updateData.carrier = dto.carrierId ? { connect: { id: dto.carrierId } } : { disconnect: true };
    }
    if (dto.priceTableId !== undefined) {
      updateData.priceTable = dto.priceTableId ? { connect: { id: dto.priceTableId } } : { disconnect: true };
    }
    if (dto.assignedUserId !== undefined) {
      updateData.assignedTo = dto.assignedUserId ? { connect: { id: dto.assignedUserId } } : { disconnect: true };
    }
    if (dto.orderTypeId !== undefined) {
      updateData.orderType = dto.orderTypeId ? { connect: { id: dto.orderTypeId } } : { disconnect: true };
    }
    if (dto.orderFlowId !== undefined) {
      updateData.orderFlow = dto.orderFlowId ? { connect: { id: dto.orderFlowId } } : { disconnect: true };
    }
    if (dto.orderModelId !== undefined) {
      updateData.orderModel = dto.orderModelId ? { connect: { id: dto.orderModelId } } : { disconnect: true };
    }

    // Se items foram fornecidos, substituir via transacao atomica (batch)
    if (dto.items !== undefined) {
      const newItems = dto.items.map((item, index) => {
        const itemTotal = Math.round(item.quantity * item.unitPriceCents) - (item.discountCents ?? 0);
        return {
          orderId: id,
          productId: item.productId,
          quantity: item.quantity,
  ...
  async changeStatus(id: string, dto: ChangeStatusDto, userId: string): Promise<OrderResponseDto> {
    const entity = await this.ordersRepository.findOrderWithRelatedEntities(id);
    if (!entity) {
      throw new NotFoundException('Pedido nao encontrado');
    }
    ...
    const context: TransitionContext = {
      ...
      productionOrders: entity.productionOrders?.map((po) => ({ status: po.status })),
      separationOrders: entity.separationOrders?.map((so) => ({ status: so.status })),
      orderItemSupplies: entity.items
        .flatMap((item) => item.supplies ?? [])
        .map((s) => ({ status: s.status })),
    };

    // Validate transition (throws on failure)
    this.statusMachine.validateTransition(orderForTransition, dto.status, context);
    ...
    // Emit event for BPM engine and side-effect listeners
  ...
  async getTimeline(id: string): Promise<OrderTimelineEntryDto[]> {
    ...
    const history = await this.ordersRepository.findStatusHistory(id);
  ...
  async registerPayment(id: string, dto: RegisterPaymentDto): Promise<OrderResponseDto> {
    const entity = await this.ordersRepository.findById(id);
    if (!entity) {
      throw new NotFoundException('Pedido nao encontrado');
    }
  ...
  async toggleSupply(
    ...
    const supply = await this.ordersRepository.findSupplyById(supplyId);
    ...
    const updated = await this.ordersRepository.updateSupply(supplyId, updateData);
