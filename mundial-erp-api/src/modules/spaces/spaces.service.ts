...
const DEFAULT_LIST_NAME = 'Processo';
...
export class SpacesService {
  ...
  async getSidebarTree(workspaceId: string) {
    ...
    const mapProcess = (l: (typeof spaces)[number]['lists'][number]) => ({
      ...
      defaultTaskTypeId: l.defaultTaskTypeId ?? null,
    });
    return spaces.map((sp) => ({
      ...
      defaultTaskTypeId: sp.defaultTaskTypeId ?? null,
      areas: sp.folders.map((f) => ({
        ...
        defaultTaskTypeId: f.defaultTaskTypeId ?? null,
  ...
  async listStatuses(workspaceId: string, spaceId: string) {
    const space = await this.spacesRepository.findById(workspaceId, spaceId);
    if (!space) {
      throw new NotFoundException('Space não encontrado');
    }
