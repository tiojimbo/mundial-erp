...
export class ProductsService {
  ...
  private async validateRelationIds(dto: {
    brandId?: string;
    ...
    if (dto.brandId) {
      checks.push(
        this.prisma.brand
          .findFirstOrThrow({ where: { id: dto.brandId, deletedAt: null } })
          .catch(() => {
            throw new BadRequestException(
              `Marca "${dto.brandId}" não encontrada`,
            );
          })
          .then(() => undefined),
      );
    ...
    if (dto.departmentCategoryId) {
      checks.push(
        this.prisma.productDepartment
          .findFirstOrThrow({
            where: { id: dto.departmentCategoryId, deletedAt: null },
          })
          .catch(() => {
            throw new BadRequestException(
              `Departamento "${dto.departmentCategoryId}" não encontrado`,
            );
          })
          .then(() => undefined),
      );
    ...
    if (dto.unitMeasureId) {
      checks.push(
        this.prisma.unitMeasure
          .findFirstOrThrow({
            where: { id: dto.unitMeasureId, deletedAt: null },
          })
          .catch(() => {
            throw new BadRequestException(
              `Unidade de medida "${dto.unitMeasureId}" não encontrada`,
            );
          })
          .then(() => undefined),
      );
    ...
    if (dto.boxUnitMeasureId) {
      checks.push(
        this.prisma.unitMeasure
          .findFirstOrThrow({
            where: { id: dto.boxUnitMeasureId, deletedAt: null },
          })
          .catch(() => {
            throw new BadRequestException(
              `Unidade de medida (caixa) "${dto.boxUnitMeasureId}" não encontrada`,
            );
          })
          .then(() => undefined),
      );
  ...
  async create(
    ...
    const createData = {
      ...
      ...(rest.departmentCategoryId && {
        departmentCategory: { connect: { id: rest.departmentCategoryId } },
      }),
      ...(rest.brandId && { brand: { connect: { id: rest.brandId } } }),
      ...(rest.unitMeasureId && {
        unitMeasure: { connect: { id: rest.unitMeasureId } },
      }),
      ...(rest.boxUnitMeasureId && {
        boxUnitMeasure: { connect: { id: rest.boxUnitMeasureId } },
      }),
    ...
    const product =
      await this.productsRepository.incrementTypeSequentialAndCreate(
        workspaceId,
        productTypeId,
        createData,
      );
  ...
  async findById(workspaceId: string, id: string): Promise<ProductResponseDto> {
    const product = await this.productsRepository.findById(workspaceId, id);
    if (!product) {
  ...
  async findByBarcode(
    ...
    const product = await this.productsRepository.findByBarcode(
      workspaceId,
      barcode,
    );
    if (!product) {
  ...
  async update(
    ...
    const existing = await this.productsRepository.findById(workspaceId, id);
    if (!existing) {
      throw new NotFoundException('Produto não encontrado');
    }
    ...
    if (dto.productionCapacity !== undefined)
      updateData.productionCapacity = dto.productionCapacity;
    if (dto.stockLocation !== undefined)
      updateData.stockLocation = dto.stockLocation;
    if (dto.minStock !== undefined) updateData.minStock = dto.minStock;
    if (dto.piecesPerUnit !== undefined)
      updateData.piecesPerUnit = dto.piecesPerUnit;
    ...
    if (dto.loadCapacity !== undefined)
      updateData.loadCapacity = dto.loadCapacity;
    ...
    if (dto.minSalePrice !== undefined)
      updateData.minSalePrice = dto.minSalePrice;
    ...
    const merged: Record<string, unknown> = {
      ...existing,
      ...this.flattenRelationUpdates(dto, existing as Record<string, unknown>),
    };
  ...
  async activate(workspaceId: string, id: string): Promise<ProductResponseDto> {
    const product = await this.productsRepository.findById(workspaceId, id);
    if (!product) {
      throw new NotFoundException('Produto não encontrado');
    }
    ...
    if (
      !product.step1Complete ||
      !product.step2Complete ||
      !product.step3Complete ||
      !product.step4Complete
    ) {
      throw new BadRequestException(
    ...
    const updated = await this.productsRepository.update(workspaceId, id, {
  ...
  async remove(workspaceId: string, id: string): Promise<void> {
    ...
    await this.productsRepository.softDelete(workspaceId, id);
  ...
  private computeStep2Complete(product: Record<string, unknown>): boolean {
    ...
    const classification =
      product.classification as ProductClassification | null;
    if (
      classification === ProductClassification.FABRICACAO_PROPRIA ||
  ...
  private flattenRelationUpdates(
    dto: UpdateProductDto,
    ...
    return {
      ...
      departmentCategoryId:
        dto.departmentCategoryId ?? existing.departmentCategoryId,
      ...
      defaultPriceTableId:
        dto.defaultPriceTableId ?? existing.defaultPriceTableId,
