...
export class UnitMeasuresRepository {
  ...
  async findById(id: string) {
    return this.prisma.unitMeasure.findFirst({ where: { id, deletedAt: null } });
  }

  async findByName(name: string) {
    return this.prisma.unitMeasure.findFirst({ where: { name, deletedAt: null } });
  }
  ...
  async findMany(params: { skip?: number; take?: number; search?: string }) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.unitMeasure.findMany({ where, skip, take, orderBy: { name: 'asc' } }),
  ...
  async softDelete(id: string) {
    return this.prisma.unitMeasure.update({ where: { id }, data: { deletedAt: new Date() } });
  }
}
