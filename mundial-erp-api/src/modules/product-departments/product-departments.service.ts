...
export class ProductDepartmentsService {
  constructor(private readonly productDepartmentsRepository: ProductDepartmentsRepository) {}

  async create(dto: CreateProductDepartmentDto): Promise<ProductDepartmentResponseDto> {
    const existing = await this.productDepartmentsRepository.findByName(dto.name);
    if (existing) {
  ...
  async update(id: string, dto: UpdateProductDepartmentDto): Promise<ProductDepartmentResponseDto> {
    const entity = await this.productDepartmentsRepository.findById(id);
    if (!entity) {
      throw new NotFoundException('Departamento de produto não encontrado');
    }
