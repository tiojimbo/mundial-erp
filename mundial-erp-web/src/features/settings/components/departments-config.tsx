...
export function DepartmentsConfig() {
  ...
  const [statusConfigDept, setStatusConfigDept] =
    useState<DepartmentConfig | null>(null);
  ...
  function handleDelete() {
    ...
    mutate.mutate(deleteTarget.id, {
      onSuccess: () => {
        notification({
          title: 'Sucesso',
          description: `${deleteTarget.type === 'dept' ? 'Departamento' : 'Área'} removido(a).`,
          status: 'success',
        });
      ...
      onError: () => {
        notification({
          title: 'Erro',
          description: `Falha ao remover ${deleteTarget.type === 'dept' ? 'departamento' : 'área'}.`,
          status: 'error',
        });
      },
    });
                      ...
                      >
                        <div className='flex items-center gap-2'>
                          ...
                          <Badge.Root
                            variant='lighter'
                            size='small'
                            color={area.isActive ? 'green' : 'gray'}
                          >
                    ...
                    {addingAreaFor === dept.id ? (
                      <div className='flex items-center gap-2'>
                        ...
                        <Button.Root
                          ...
                          mode='filled'
                          size='small'
                          onClick={() => handleCreateArea(dept.id)}
                        >
                          Criar
                        </Button.Root>
                        <Button.Root
      ...
      {deleteTarget && (
        <Modal.Root open onOpenChange={() => setDeleteTarget(null)}>
          <Modal.Content>
            <Modal.Header>
              <Modal.Title>
                Remover {deleteTarget.type === 'dept' ? 'Departamento' : 'Área'}
              </Modal.Title>
              <Modal.Description>
