...
export function ProcessesConfig() {
  ...
  const [addingActivityFor, setAddingActivityFor] = useState<string | null>(
    null,
  );
  ...
  function handleCreateActivity(processId: string) {
    ...
    const existingActivities =
      (processes ?? []).find((p) => p.id === processId)?.activities ?? [];
  ...
  function handleDelete() {
    ...
    const mutate =
      deleteTarget.type === 'process' ? deleteProcess : deleteActivity;
    mutate.mutate(deleteTarget.id, {
      onSuccess: () => {
        notification({
          title: 'Sucesso',
          description: `${deleteTarget.type === 'process' ? 'Processo' : 'Atividade'} removido(a).`,
          status: 'success',
        });
      ...
      onError: () => {
        notification({
          title: 'Erro',
          description: `Falha ao remover ${deleteTarget.type === 'process' ? 'processo' : 'atividade'}.`,
          status: 'error',
        });
      },
    });
    ...
    <div className='space-y-4'>
        ...
        <div className='flex items-end gap-3 rounded-xl border border-stroke-soft-200 bg-bg-weak-50 p-4'>
          <div className='flex-1 space-y-1.5'>
            ...
            <input
              ...
              className='shadow-xs w-full rounded-lg border border-stroke-soft-200 bg-bg-white-0 px-3 py-2 text-paragraph-sm'
            />
          </div>
      ...
      <div className='space-y-2'>
        {(processes ?? []).map((process) => {
          ...
          const sortedActivities = [...process.activities].sort(
            (a, b) => a.order - b.order,
          );
                ...
                <div className='flex-1'>
                  <div className='flex items-center gap-2'>
                    ...
                    <Badge.Root
                      variant='lighter'
                      size='small'
                      color={process.isActive ? 'green' : 'gray'}
                    >
                  ...
                  <p className='text-paragraph-xs text-text-sub-600'>
                    ...
                    {isBpm
                      ? ` - ${process.activities.length} atividade(s)`
                      : ''}
                  </p>
                </div>
                <button
                  ...
                  className='rounded-md p-1.5 text-text-sub-600 hover:bg-error-lighter hover:text-error-base disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-text-sub-600'
                >
                      ...
                      >
                        ...
                        <div className='flex items-center gap-2'>
                          <Badge.Root
                            variant='lighter'
                            size='small'
                            color={activity.isActive ? 'green' : 'gray'}
                          >
                    ...
                    {addingActivityFor === process.id ? (
                      <div className='flex items-center gap-2'>
                        <input
                          ...
                          className='shadow-xs flex-1 rounded-lg border border-stroke-soft-200 bg-bg-white-0 px-3 py-2 text-paragraph-sm'
                          autoFocus
                        />
                        <input
                          ...
                          placeholder='SLA (h)'
                          type='number'
                          className='shadow-xs w-24 rounded-lg border border-stroke-soft-200 bg-bg-white-0 px-3 py-2 text-paragraph-sm'
                        />
                        <Button.Root
                          variant='primary'
                          mode='filled'
                          size='small'
                          onClick={() => handleCreateActivity(process.id)}
                        >
                          Criar
                        </Button.Root>
                        <Button.Root
        ...
        <Modal.Root open onOpenChange={() => setDeleteTarget(null)}>
          <Modal.Content>
            <Modal.Header>
              <Modal.Title>
                Remover{' '}
                {deleteTarget.type === 'process' ? 'Processo' : 'Atividade'}
              </Modal.Title>
              <Modal.Description>
...
function DefaultTaskTypeRow({ process }: { process: ProcessConfig }) {
  ...
  return (
    <div className='flex items-center gap-3'>
      <label className='w-32 text-label-sm text-text-strong-950'>
        Tipo padrão
      </label>
      <select
        value={currentId}
        ...
        className='shadow-xs flex-1 rounded-lg border border-stroke-soft-200 bg-bg-white-0 px-3 py-2 text-paragraph-sm disabled:opacity-50'
      >
