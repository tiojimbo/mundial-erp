...
export function SubtaskRow({ task, onToggleComplete }: SubtaskRowProps) {
  ...
  return (
    <div className='border-border/60 hover:bg-muted/40 flex items-center gap-2 rounded-lg border px-3 py-2 transition-colors duration-150'>
      <button
        type='button'
        aria-label={done ? 'Marcar como nao concluida' : 'Concluir subtarefa'}
        aria-pressed={done}
        onClick={onToggleComplete}
        className={`h-4 w-4 rounded-full border ${done ? 'border-primary bg-primary' : 'border-muted-foreground/60 border-dashed hover:border-foreground'}`}
      />
      <a
        href={`/tasks/${task.id}`}
