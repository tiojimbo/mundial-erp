...
export function SyncDashboard() {
  ...
  const sortedEntities = useMemo(() => {
    ...
    return [...entities].sort(
      (a, b) =>
        (SYNC_ORDER_MAP.get(a.entity) ?? 99) -
        (SYNC_ORDER_MAP.get(b.entity) ?? 99),
    );
  }, [status?.entities]);
  ...
  return (
    <div className='space-y-8 p-6'>
      ...
      <div className='flex items-center gap-4'>
        <div className='shadow-xs flex size-12 items-center justify-center rounded-full border border-stroke-soft-200'>
          <RiExchangeLine className='size-6 text-text-sub-600' />
        </div>
        <div>
          <h1 className='text-label-lg text-text-strong-950'>
            Integração Pro Finanças
          </h1>
        ...
        <div className='mb-3 flex items-center justify-between'>
          ...
          <div className='flex gap-2'>
            <select
              ...
              onChange={(e) =>
                setLogEntityFilter((e.target.value as SyncEntity) || undefined)
              }
              className='shadow-xs rounded-lg border border-stroke-soft-200 bg-bg-white-0 px-3 py-1.5 text-paragraph-sm text-text-sub-600'
            >
            ...
            <select
              ...
              onChange={(e) =>
                setLogStatusFilter(
                  (e.target.value as SyncStatusType) || undefined,
                )
              }
              className='shadow-xs rounded-lg border border-stroke-soft-200 bg-bg-white-0 px-3 py-1.5 text-paragraph-sm text-text-sub-600'
            >
