...
export function MembersPopover({
  ...
  const { mutate: addMembers, isPending: isAdding } = useAddMembers(channelId);
  const { user } = useAuth();

  const { data: usersData } = useUsers(
    mode === 'add'
      ? { limit: 10000, sortBy: 'name', sortOrder: 'asc' }
      : undefined,
  );
  ...
  const nonMembers = useMemo(() => {
    ...
    const outside = all.filter((u) => !memberUserIds.has(u.id));
    if (!searchInput.trim()) return outside;
            ...
            <div className='flex items-center justify-between px-4 pb-2 pt-4'>
              <div className='flex items-center gap-2'>
                ...
                <h3 className='text-label-sm font-semibold text-text-strong-950'>
                  {mode === 'add' ? 'Adicionar Pessoas' : 'Observadores'}
                </h3>
              </div>
              <button
                type='button'
              ...
              <input
                ...
                placeholder={
                  mode === 'add'
                    ...
                    : 'Buscar pessoas ou convidar por email'
                }
                value={searchInput}
