...
export function FavoriteChip({ favorite }: FavoriteChipProps) {
  ...
  const content = (
    <>
      <Icon
        className='group-hover/fav:text-primary size-3 shrink-0 text-muted-foreground transition-colors'
        aria-hidden
      />
  ...
  return (
    <div className='group/fav border-border/60 bg-muted/40 relative flex h-6 shrink-0 items-center rounded-lg border transition-all duration-150 hover:border-border hover:bg-muted hover:shadow-[0_1px_2px_rgba(0,0,0,0.04)] active:scale-[0.97] active:cursor-grabbing'>
      {href ? (
        <Link
          href={href}
          title={label}
          className='text-foreground/70 flex h-full items-center gap-1.5 px-2 text-[11px] transition-colors group-hover/fav:text-foreground'
        >
          {content}
        </Link>
      ) : (
        <span
          title={label}
          className='text-foreground/70 flex h-full items-center gap-1.5 px-2 text-[11px]'
        >
          {content}
        </span>
      )}
      <div className='pointer-events-none absolute inset-y-0 right-0 flex items-center rounded-r-lg bg-muted pl-3 pr-1 opacity-0 transition-opacity group-hover/fav:pointer-events-auto group-hover/fav:opacity-100'>
        <button
          ...
          className='hover:bg-background/60 flex size-5 items-center justify-center rounded-sm text-muted-foreground transition-colors hover:text-destructive disabled:opacity-50'
        >
