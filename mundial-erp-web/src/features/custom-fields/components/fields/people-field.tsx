              ...
              {selectedMembers.length > 0 ? (
                <div className="flex items-center -space-x-1.5">
                  ...
                  {selectedMembers.length > 5 && (
                    <span className="bg-muted text-muted-foreground ring-background relative flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold ring-2">
                      +{selectedMembers.length - 5}
                    </span>
                  )}
                </div>
...
function Avatar({ member, ring }: { member: NormalizedMember; ring?: boolean }) {
  const c = colorOf(member.id);
  const ringCls = ring ? 'ring-background ring-2' : '';
  if (member.avatarUrl) {
    return (
      <span className={`relative flex h-6 w-6 shrink-0 overflow-hidden rounded-full ${ringCls}`}>
