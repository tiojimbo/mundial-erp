...
export function ManagerLocationCell({ def }: ManagerLocationCellProps) {
  ...
  const replaceLocation = async (
    ...
    try {
      ...
      toast.success(current ? 'Localização alterada.' : 'Campo vinculado ao local.');
      setOpen(false);
    } catch {
    ...
    <div className="relative inline-block">
      ...
      {open ? (
        <>
          ...
          />
          <div className="bg-popover absolute left-0 top-full z-50 mt-1 w-[320px] rounded-md border shadow-md">
              ...
              <Command className="flex h-full w-full flex-col overflow-hidden rounded-md bg-popover text-popover-foreground">
                  <div className="flex h-9 items-center gap-2 border-b px-3">
                    ...
                    <Command.Input
                      ...
                      className="placeholder:text-muted-foreground flex h-10 w-full bg-transparent py-3 text-paragraph-sm outline-none"
                    />
                  </div>
                  <Command.List className="max-h-[200px] overflow-auto">
                    ...
                    <LocGroup
                      ...
                      items={flatOptions.lists.filter(
                        (l) =>
                          !(current?.type === 'list' && current.id === l.id),
                      )}
                      onSelect={replaceLocation}
                    />
                  </Command.List>
    ...
    >
      {items.map((it) => (
        <Command.Item
          ...
          className="data-[selected=true]:bg-accent relative flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1.5 text-paragraph-sm outline-none"
        >
