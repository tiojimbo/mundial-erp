...
import { useRegisterAPPayment } from '../hooks/use-financial';
...
export function APPaymentDialog({ apId, balanceCents, onClose }: Props) {
  const [amountReais, setAmountReais] = useState('');
  const [paidDate, setPaidDate] = useState(
    new Date().toISOString().split('T')[0],
  );
  const mutation = useRegisterAPPayment(apId);
