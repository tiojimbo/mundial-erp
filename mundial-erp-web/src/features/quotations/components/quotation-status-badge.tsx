...
const QUOTATION_BADGE_MAP: Record<
  QuotationStatus,
  { color: BadgeColor; variant: 'filled' | 'light' | 'lighter' }
> = {
...
const PURCHASE_ORDER_BADGE_MAP: Record<
  PurchaseOrderStatus,
  { color: BadgeColor; variant: 'filled' | 'light' | 'lighter' }
> = {
...
export function QuotationStatusBadge({ status }: { status: QuotationStatus }) {
  const config = QUOTATION_BADGE_MAP[status] ?? {
...
export function PurchaseOrderStatusBadge({
  ...
  const config = PURCHASE_ORDER_BADGE_MAP[status] ?? {
    color: 'gray' as BadgeColor,
    variant: 'lighter' as const,
  };
