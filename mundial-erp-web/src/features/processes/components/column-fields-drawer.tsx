...
import { useProcessViews, processViewsKeys } from '@/features/process-views/hooks/use-process-views';
...
export function ColumnFieldsDrawer({ open, onClose, listId }: ColumnFieldsDrawerProps) {
  ...
  const visibleIds = useMemo(() => {
    ...
    return new Set(Array.isArray(cfg?.visibleCustomFields) ? cfg!.visibleCustomFields : []);
  }, [listView]);
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(o) => !o && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="bg-background fixed inset-y-0 right-0 z-50 flex h-full w-full max-w-sm flex-col border-l p-0 shadow-lg data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right data-[state=closed]:duration-300 data-[state=open]:duration-500">
          <header className="flex flex-col gap-1.5 px-6 pt-6 pb-2">
            <Dialog.Title className="text-foreground font-semibold">Campos</Dialog.Title>
            ...
            <Dialog.Close asChild>
              <button
                type="button"
                aria-label="Fechar"
                className="text-muted-foreground hover:bg-accent absolute right-4 top-4 inline-flex size-7 items-center justify-center rounded-md transition-colors"
              >
                <X className="h-4 w-4" />
              </button>
            </Dialog.Close>
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-5 px-6 pb-2">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              {definitionsQuery.isLoading && (
                <p className="text-muted-foreground text-paragraph-sm">Carregando…</p>
              )}
                ...
                <div key={group.key} className="mb-6">
                  ...
                  {group.defs.map((def) => {
                    ...
                    return (
                      <div
                        key={def.id}
                        className="text-foreground flex items-center justify-between gap-2 py-2"
                      >
                        ...
                        >
                          <Switch.Thumb className="bg-background pointer-events-none block h-5 w-5 rounded-full shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-5 data-[state=unchecked]:translate-x-0" />
                        </Switch.Root>
                      </div>
                    );
                  })}
